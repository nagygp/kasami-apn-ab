import AlmostBent.WalshLayers
import AlmostBent.CrossForm
import AlmostBent.WalshDiv
-------------------------------------------
...
theorem kasami_is_apn_pred {n : ℕ} (k : ℕ) (hk : k ≥ 1)
    (hcop : Nat.Coprime k n) (hkn : k < n) :
  IsAPN (fun x : GeneralizedKasami.GF n => x ^ d k) := by
  intro a ha b
  exact kasami_apn hk hkn hcop a ha b
...
theorem kasami_is_ab {n : ℕ} (hcard : Fintype.card (GeneralizedKasami.GF n) = 2 ^ n)
    (k : ℕ) (hk : k ≥ 1) (hcop : Nat.Coprime k n) (hnodd : Odd n)
  (hn : n ≥ 1) (hkn : k < n) : IsAB hcard (fun x : GeneralizedKasami.GF n => x ^ d k) := by
  ...
  · exact fun a ha => fourth_moment_apn hcard (d k)
      ...
      (kasami_is_apn_pred k hk hcop hkn) a ha
