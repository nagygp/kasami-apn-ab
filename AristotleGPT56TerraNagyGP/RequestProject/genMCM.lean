...
/-- The Muller--Cohen--Matthews polynomial is a permutation polynomial when
`k` is odd and coprime to `n`.  This is the `β = 0` case of Dobbertin's
Theorem 4. -/
theorem mcm_isPermutation
    (n k : ℕ)
    (hn : 0 < n) (hk : 0 < k) (hkn : k < n)
    (hcop : Nat.Coprime k n) (hkOdd : k % 2 = 1) :
    IsPermutationPolynomial (genMCMpol n k 0) := by
  obtain ⟨kInv, hInvLt, hInvMod⟩ :=
      Nat.exists_mul_mod_eq_one_of_coprime hcop (by omega)
  let nPrime := (kInv * k) / n
  have hprod : kInv * k = nPrime * n + 1 := by
    have hdiv := Nat.mod_add_div (k * kInv) n
    calc
      kInv * k = k * kInv := by ac_rfl
      _ = (k * kInv) % n + n * ((k * kInv) / n) := by omega
      _ = nPrime * n + 1 := by simp [nPrime, hInvMod, mul_comm, add_comm]
  let a : Fin 2 := ⟨nPrime % 2, Nat.mod_lt _ (by omega)⟩
  have hInvPos : 0 < kInv := by
    by_contra h
    have : kInv = 0 := by omega
    simp [this] at hInvMod
  have hkMod : k ≡ 1 [MOD 2] := by
    simp [Nat.ModEq, hkOdd]
  have haMod : a.val ≡ nPrime [MOD 2] := by
    simp [a, Nat.ModEq]
  have hleft : kInv ≡ nPrime * n + 1 [MOD 2] := by
    calc
      kInv ≡ kInv * k [MOD 2] := by
        simpa using (Nat.ModEq.mul_left kInv hkMod).symm
      _ ≡ nPrime * n + 1 [MOD 2] := hprod ▸ Nat.ModEq.refl _
  have hqpar : kInv + a.val * n ≡ 1 [MOD 2] := by
    calc
      kInv + a.val * n ≡ kInv + nPrime * n [MOD 2] :=
        Nat.ModEq.add_left kInv (Nat.ModEq.mul_right n haMod)
      _ ≡ (nPrime * n + 1) + nPrime * n [MOD 2] :=
        Nat.ModEq.add_right (nPrime * n) hleft
      _ ≡ 2 * (nPrime * n) + 1 [MOD 2] := by
        rw [show (nPrime * n + 1) + nPrime * n = 2 * (nPrime * n) + 1 by ring]
      _ ≡ 1 [MOD 2] := by simp [Nat.ModEq]
  have hbeta : (0 : Fin 2).val ≡ nPrime + a.val * k [MOD 2] := by
    have haK : a.val * k ≡ nPrime [MOD 2] := by
      calc
        a.val * k ≡ nPrime * k [MOD 2] := Nat.ModEq.mul_right k haMod
        _ ≡ nPrime * 1 [MOD 2] := Nat.ModEq.mul_left nPrime hkMod
        _ ≡ nPrime [MOD 2] := by
          simpa using (Nat.ModEq.refl nPrime : nPrime ≡ nPrime [MOD 2])
    calc
      (0 : Fin 2).val ≡ nPrime + nPrime [MOD 2] := by
        rw [show nPrime + nPrime = 2 * nPrime by ring]
        simp [Nat.ModEq]
      _ ≡ nPrime + a.val * k [MOD 2] := Nat.ModEq.add_left nPrime haK.symm
  exact (dobbertin_theorem4 n k kInv nPrime a 0 hn hk hkn hInvPos hInvLt
    hcop hprod hqpar hbeta).2.2.2.2.2
